...
SOURCES		:=	$(UNIVCORE) source source/paint
DATA		:=	data
INCLUDES	:=	$(UNIVCORE) source source/paint
GRAPHICS	:=	gfx
#GFXBUILD	:=	$(BUILD)
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
APP_AUTHOR	:=	Tobi-D7
APP_DESCRIPTION :=      Paint program for 3ds 
ICON		:=	app/icon.png
...
$(OUTPUT).cia	:	$(OUTPUT).elf $(OUTPUT).smdh
	...
	$(MAKEROM) -f cia -target t -exefslogo -o "../Paint3ds.cia" -elf "../Paint3ds.elf" -rsf "../app/build-cia.rsf" -banner "../app/banner.bin" -icon "../app/icon.bin" -logo "../app/splash.lz" -DAPP_ROMFS="$(TOPDIR)/$(ROMFS)" -major $(VERSION_MAJOR) -minor $(VERSION_MINOR) -micro $(VERSION_MICRO) -DAPP_VERSION_MAJOR="$(VERSION_MAJOR)"
#---------------------------------------------------------------------------------
